...
function getConf (argv) {
	...
	const npmrc = {
		...
		'fuse-bindings-binary-host-mirror':
			'{bin-mirrors}/fuse-bindings/v{version}',
	...
	const opts = {
		...
		binMirrorsPrefix: 'http://npm.sankuai.com/mirrors',
		...
		Object.keys(obj).forEach(key => {
			if (obj[key]) {
				obj[key] = obj[key].replace(
					/\{(\w+)-mirrors\}/g,
					(s, prefix) => opts[prefix + 'MirrorsPrefix'] || s
				);
			}
		});
		return obj;
	...
	return (isWin && (!env.http_proxy || !env.https_proxy)
		? reg
			.query(
			...
			.then(settings => {
				if (settings.autoConfigURL) {
					env.https_proxy = settings.autoConfigURL.replace(
						/^(\w+:\/\/[^/]+).*/,
						'$1'
					);
